...
DATA_DIRECTORY = 'G:/seq2point-nilm/housedata/refit/CLEAN_REFIT_081116/'
SAVE_PATH = 'kettle/'
AGG_MEAN = 522
AGG_STD = 814
def get_arguments():
...
def main():
    ...
    for idx, filename in enumerate(os.listdir(path)):
        ...
        if filename == 'CLEAN_House' + str(params_appliance[appliance_name]['test_house']) + '.csv':
            ...
            # Save
            test.to_csv(save_path + appliance_name + '_test_' + '.csv', index=False)
            ...
            # Save
            val.to_csv(save_path + appliance_name + '_validation_' + '.csv', index=False)
